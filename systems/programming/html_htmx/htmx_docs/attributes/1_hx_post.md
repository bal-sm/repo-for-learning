+++
title = "`hx-post` - Mahmuda's version"
+++

- The `hx-post` attribute will cause an element
  - to issue a `POST` to the specified URL
  - and swap the HTML into the DOM
...
- This example will cause the `button`
  - to issue a `POST` to `/account/enable`
...
- You can control what event triggers
  - the request with the [`hx-trigger`](@/attributes/hx-trigger.md) attribute
